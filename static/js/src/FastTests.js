;(function testesAG() {
  ...
  var config = {
    ...
    funcaoAfinidade: function (individuo) {
      var acumulador = 0
      individuo.genoma.split('').forEach(function (s) {
        acumulador += parseInt(s)
      })
      return acumulador
    },
  }
  ...
  console.assert(
    execucao.populacao[0].genoma.length == 3,
    'Tamanho do genoma dos indiíduos deve ser 3'
  )
  console.assert(
    execucao.populacao[0].genoma[0] == '0' ||
      execucao.populacao[0].genoma[0] == '1' ||
      execucao.populacao[0].genoma[0] == '2',
    'Genoma deve ser composto apenas por genes permitidos 0,1 ou 2'
  )
  ...
  console.assert(
    execucao.populacao[0].diversidade === 1,
    'Deversidade do individuo 0 deveria ser 2'
  )
  console.assert(
    execucao.populacao[1].diversidade === 2,
    'Deversidade do individuo 1 deveria ser 3'
  )
  console.assert(
    execucao.populacao[2].diversidade === 1,
    'Deversidade do individuo 2 deveria ser 0'
  )
  console.assert(
    execucao.populacao[3].diversidade === 1,
    'Deversidade do individuo 3 deveria ser 3'
  )
  console.assert(
    execucao.populacao[4].diversidade === 1,
    'Deversidade do individuo 4 deveria ser 0'
  )
  console.assert(
    execucao.populacao[5].diversidade === 1,
    'Deversidade do individuo 5 deveria ser 0'
  )
  ...
  console.assert(
    execucao.populacao[1].fitnessNormalizadoAcumulado == 0.25,
    'Fitness deveria ser 0.25'
  )
  ...
  console.assert(
    amostra.melhorIndividuo.fitness == 3,
    'O melhorIndividuo indivíduo deve ser o melhor'
  )
  ...
  populacaoAnterior.forEach(function (old, i) {
    if (old.genoma != execucao.populacao[i].genoma) novosIndividuosDiferentes++
  })
  console.assert(
    execucao.populacao.length == populacaoAnterior.length,
    'A nova geração deve ter tantos indivíduos quanto a geracao anterior'
  )
