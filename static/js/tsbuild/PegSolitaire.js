...
class BoardSlot {
    ...
    possibleMoves(boardSlots) {
        return this.validMoves.filter(([from, mid, to]) => !boardSlots[from].empty() && !boardSlots[mid].empty() && boardSlots[to].empty());
    }
    reset() {
...
class Board {
    constructor(config = {}) {
        ...
        this.solutionFinderWorker = null;
        // prettier-ignore
        this.slots = [
        ...
        this.centralHole = this.slots[16];
        const initialSequence = config.sequence || '';
        delete config.sequence;
        Object.assign(this, config);
        const slotw = Math.floor(this.width / 7), dslotw = Math.floor(slotw / 2), sloth = Math.floor(this.height / 7), dsloth = Math.floor(sloth / 2);
    ...
    createView() {
        ...
        if (this.compactView) {
            ...
            g.append('svg:text')
                .attr('x', '-0.5em')
                .attr('dy', '.31em')
                .text((_d, i) => `${i.toString()}`.padStart(2, '0'));
            svg
                .append('svg:text')
